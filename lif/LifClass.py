...
class LifFile:
    ...
    def get_metadata(self,save=True):
        ...
        print('Creating subfolders in ' + project_path + '...')
    ...
    def get_proj(self):
        ...
        if (self.md.empty):
            ...
            print('Looking at ' + self.fullpath + '...')
            project_path = os.path.join(self.dpath,self.date,self.project)
            rdr = bf.ImageReader(self.fullpath)
            for i in range(self.md.shape[0]):  # Loop through series
                print('--Calculating and saving z projections for ' + self.md.Name[i] + '...')
                stack = []
                ...
                folder_name = 'S{:0>2d}'.format(i+1) + '_' + self.md.Name[i]
                cv2.imwrite(os.path.join(project_path,folder_name,'z_proj.png'),im_out)
