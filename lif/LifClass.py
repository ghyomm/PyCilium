...
class LifFile:
    ...
    def get_metadata(self, save=True):
        ...
        for ind in range(ome.image_count):
            ...
            md.append([iome.get_ID(), iome.get_Name(), iome.Pixels.get_SizeC(),
                       iome.Pixels.get_SizeX(), iome.Pixels.get_PhysicalSizeX(),
                       iome.Pixels.get_PhysicalSizeXUnit(),
                       iome.Pixels.get_SizeY(), iome.Pixels.get_PhysicalSizeY(),
                       iome.Pixels.get_PhysicalSizeYUnit(),
                       iome.Pixels.get_SizeZ(), iome.Pixels.get_PhysicalSizeZ(),
    ...
    def get_proj(self):
        ...
        if self.md.empty:
            ...
            # Remi: Why not then call self.get_metadata() here and continue
        else:
            project_path = os.path.join(self.dpath, self.date, self.project)

            for i in range(self.md.shape[0]):  # Loop through series
                series_folder = 'S{:0>2d}'.format(i + 1) + '_' + self.md.Name[i]
                stack = self.get_serie_stack(i)
                # proj_list = []
                # Save one z projection image per channel (if necessary)
                for c in range(self.md.Nchan[i]):
                    proj_file = os.path.join(project_path, series_folder,
                                             'z_proj_chan' + str(c + 1) + '.png')
                    if not os.path.exists(proj_file):
                        print('Saving channel ' + str(c + 1) + ' z projection for ' +
                              self.md.Name[i] + '...')
